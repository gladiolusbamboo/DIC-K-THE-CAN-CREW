...
postMessage(result);};FormatterWorker.findLastExpression=function(content){if(content.length>10000)
...
let ast;const suffix='.DEVTOOLS';try{acorn.parse(content+suffix,{ecmaVersion:9});}catch(parseError){if(parseError.message.startsWith('Unexpected token')&&parseError.pos===content.length)
return null;}
let parsedContent='';for(let i=0;i<content.length;i++){try{parsedContent=content[i]==='{'?`(${content.substring(i)})${suffix}`:`${content.substring(i)}${suffix}`;ast=acorn.parse(parsedContent,{ecmaVersion:9});break;}catch(e){}}
if(!ast)
return null;const types=new Set(['MemberExpression','Identifier']);let baseNode=null;const walker=new FormatterWorker.ESTreeWalker(node=>{if(baseNode||node.end<ast.end)
